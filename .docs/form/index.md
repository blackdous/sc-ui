...
<demo src="./demo/excludeSelect.vue"
  language="vue"
  title="互斥多选"
  >
</demo>
...
<demo src="./demo/advancedForm.vue"
  language="vue"
  title="可收缩表单"
  >
</demo>
...
<demo src="./demo/customerForm.vue"
  language="vue"
  title="自定义组件"
  >
</demo>
...
```tsx
import { useComponentRegister } from 'sc-ui';
  ...
  <div class="m-4">
    <ScForm @register="register">
      <template #customSlot="{ model, field }">
        <ScInput v-model:value="model[field]" />
      </template>
    </ScForm>
  </div>
</template>
<script lang="ts">
  ...
  import { ScForm, useForm, ScModal, ScInput } from 'sc-ui';
  export default defineComponent({
    name: 'FormDemo',
    setup(props) {
